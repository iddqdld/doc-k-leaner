...
const AuditDashboard: React.FC = () => {
  ...
  const cards = useMemo(
    () =>
      stats
        ? [
            { label: 'Total Files', value: stats.total_files, color: 'bg-indigo-500', icon: 'FI' },
            { label: 'Total Scans', value: stats.total_scans, color: 'bg-blue-500', icon: 'SC' },
            { label: 'Critical', value: stats.critical, color: 'bg-red-500', icon: 'CR' },
            { label: 'High', value: stats.high, color: 'bg-orange-500', icon: 'HI' },
            { label: 'Medium', value: stats.medium, color: 'bg-amber-500', icon: 'ME' },
            { label: 'Low', value: stats.low, color: 'bg-emerald-500', icon: 'LO' },
          ]
        : [],
    [stats]
  );
